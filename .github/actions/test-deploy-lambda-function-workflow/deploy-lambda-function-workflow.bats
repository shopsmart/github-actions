...
@test "it should have created a current version file" {
  run aws s3 cp "s3://$S3_BUCKET/.current-versions/staging" "$BATS_TEST_TMPDIR/current-version"

  [ "$status" -eq 0 ]
  [ -f "$BATS_TEST_TMPDIR/current-version" ]
  [ "$(< "$BATS_TEST_TMPDIR/current-version")" = "$VERSION_TAG" ]
}
